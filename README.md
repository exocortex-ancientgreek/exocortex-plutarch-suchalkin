# Плутарх. Порівняльні життєписи

Читальне видання **«Порівняльних життєписів» Плутарха** (вибрані фрагменти з коментарем
та греко-українським словником), автор-укладач **О. В. Сучалкін** — ХНУ ім. В. Н. Каразіна, 2021.
Опубліковано з дозволу автора.

Це той самий текст, коментар і словник, перетворені на читальний інтерфейс: клік по слову
показує його коментар, словник сторінки стоїть поруч із текстом, подвійний клік віддає слово
Alpheios для морфології. Кнопка **«Вступ»** відкриває передмову і вступ видання.

- **Сайт:** https://exocortex-ancientgreek.github.io/exocortex-plutarch-suchalkin/
- **Автор:** [сайт](https://suchalkin.wixsite.com/greek/peri-emaytoy) · [публікації](https://suchalkin.wixsite.com/greek/naukova-literatura)
- **Оригінали (PDF):** [текст видання](https://59500d52-1b27-43fb-928c-485a0c9aeebe.filesusr.com/ugd/96a2c7_4184c044e4c2420f9ef86ea59ffe0232.pdf) · [«Давньогрецько-український словник» (ХНУ, 2015)](https://59500d52-1b27-43fb-928c-485a0c9aeebe.filesusr.com/ugd/96a2c7_890e606f371a4d4abe7f873c2ec93e1c.pdf) —
  надчастотні леми, яких немає у словнику цього видання, доповнено саме з нього (того ж автора).
- **Джерело збірки:** генерується із застосунку GreekMultitool
  (`tools/site/build-plutarch-site.mjs`).

## Ліцензія

Грецький текст — суспільне надбання. Словник і коментар © О. В. Сучалкін, використано з дозволу
автора. Ця читальна версія поширюється на тих самих умовах. Не афілійовано з автором і не
схвалено ним окремо.

[GitHub](https://github.com/exocortex-ancientgreek/exocortex-plutarch-suchalkin) · [Telegram](https://t.me/exocortex_ancientgreek) · [YouTube](https://www.youtube.com/@exocortex_ancientlanguages)
